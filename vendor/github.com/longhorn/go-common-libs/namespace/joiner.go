...
func (joiners Joiners) ReverseOrder() Joiners {
	reversed := make(Joiners, len(joiners))
	for i, j := len(joiners)-1, 0; i >= 0; i, j = i-1, j+1 {
...
// RunFunc runs the given function in the specified process namespace.
// Returns the result of the function and any error that occurred.
func RunFunc(fn func() (interface{}, error), procName, procDir string, timeout time.Duration) (interface{}, error) {
	joiner, err := NewJoiner(procName, types.HostProcDirectory, 0)
	if err != nil {
		return nil, err
	}
...
type NewJoinerFunc func(string, string, time.Duration) (JoinerInterface, error)
...
func newJoiner(procName string, procDirectory string, timeout time.Duration) (nsjoin JoinerInterface, err error) {
	log := logrus.WithFields(logrus.Fields{
		"procDirectory": procDirectory,
		"timeout":       timeout,
	})
	if procName != "" {
		log = log.WithField("procName", procName)
	}
	...
	nsDir := utils.GetHostNamespaceDirectory(procDirectory)
	procPid := utils.GetHostNamespacePID(procDirectory)
	if procName != types.ProcessNone {
		nsDir, err = utils.GetProcessNamespaceDirectory(procName, procDirectory)
		if err != nil {
			return nil, err
		}

		var procPids []uint64
		procPids, err = utils.GetProcessPIDs(procName, procDirectory)
		if err != nil {
			return nil, err
		}
		procPid = procPids[0]
