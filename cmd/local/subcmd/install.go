...
This can help to ensure that your Kubernetes cluster is properly configured and meets the necessary requirements for Longhorn.`,
	...
	cmd.Flags().BoolVar(&localInstaller.EnableSpdk, consts.CmdOptEnableSpdk, utils.ConvertStringToTypeOrDefault(os.Getenv(consts.EnvEnableSpdk), true), "Enable installation of SPDK required packages, modules, and setup.")
	cmd.Flags().StringVar(&localInstaller.SpdkOptions, consts.CmdOptSpdkOptions, os.Getenv(consts.EnvSpdkOptions), "Specify a space-separated list of custom options for configuring SPDK environment.")
	...
	cmd.Flags().BoolVar(&localInstaller.AllowPci, consts.CmdOptAllowPci, utils.ConvertStringToTypeOrDefault(os.Getenv(consts.EnvPciAllowed), true), "Specify a space-separated list of allowed PCI devices. By default, all PCI devices are blocked by a non-valid address.")
	cmd.Flags().StringVar(&localInstaller.DriverOverride, consts.CmdOptDriverOverride, os.Getenv(consts.EnvDriverOverride), "User space driver for device bindings.")
