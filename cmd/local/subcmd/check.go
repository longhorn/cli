...
These checks can help to identify issues that might prevent Longhorn from functioning properly.`,
	...
	cmd.Flags().BoolVar(&localChecker.EnableSpdk, consts.CmdOptEnableSpdk, utils.ConvertStringToTypeOrDefault(os.Getenv(consts.EnvEnableSpdk), true), "Enable installation of SPDK required packages, modules, and setup.")
