...
func (local *Installer) Run() error {
	...
	if err := local.probeModules(false); err != nil {
		return err
	}
	...
	if local.EnableSpdk {
		...
		if err := local.probeModules(true); err != nil {
			return err
		}
...
func (local *Installer) probeModules(spdkDependent bool) error {
...
func getArgsForConfiguringSPDKEnv(options string) []string {
	...
	if options != "" {
		...
		customOptions := strings.Split(options, " ")
