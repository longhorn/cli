...
- --volume-name: Filter replicas based on the volume they belong to.`,
	...
	cmd.Flags().StringVar(&localGetter.ReplicaName, consts.CmdOptName, os.Getenv(consts.EnvLonghornReplicaName), "Specify the name of the replica to retrieve information (optional).")
	cmd.Flags().StringVar(&localGetter.VolumeName, consts.CmdOptLonghornVolumeName, os.Getenv(consts.EnvLonghornVolumeName), "Specify the name of the volume to retrieve replica information (optional).")
	cmd.Flags().StringVar(&localGetter.LonghornDataDirectory, consts.CmdOptLonghornDataDirectory, os.Getenv(consts.EnvLonghornDataDirectory), "Specify the Longhorn data directory. If not provided, the default will be attempted, or it will fall back to the directory of longhorn-disk.cfg (optional).")
