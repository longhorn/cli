...
func NewCmdExportReplica(globalOpts *types.GlobalCmdOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `This command exports the data stored within a specific Longhorn replica to a directory on the host machine.
This command offers valuable data recovery capabilities when the Longhorn system is unavailable.`,
	...
	cmd.Flags().StringVar(&replicaExporter.LonghornDataDirectory, consts.CmdOptLonghornDataDirectory, "/var/lib/longhorn", "Specify the Longhorn data directory. If not provided, the default will be attempted, or it will fall back to the directory of longhorn-disk.cfg (optional).")
