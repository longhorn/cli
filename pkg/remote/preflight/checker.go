...
func (remote *Checker) Cleanup() error {
	if err := lhgokube.DeleteDaemonSet(remote.kubeClient, remote.namespace, remote.appName); err != nil {
		return errors.Wrapf(err, "Failed to delete daemonset %v", remote.appName)
	}

	if err := lhgokube.DeleteClusterRoleBinding(remote.kubeClient, remote.appName); err != nil {
		return errors.Wrapf(err, "Failed to delete cluster role binding %v", remote.appName)
	}

	if err := lhgokube.DeleteClusterRole(remote.kubeClient, remote.appName); err != nil {
		return errors.Wrapf(err, "Failed to delete cluster role %v", remote.appName)
	}

	if err := lhgokube.DeleteServiceAccount(remote.kubeClient, remote.namespace, remote.appName); err != nil {
		return errors.Wrapf(err, "Failed to delete service account %v", remote.appName)
	}
